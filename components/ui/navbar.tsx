...
const navStyles = tv({
  ...
  variants: {
    ...
    intent: {
      ...
      navbar:
        "border-fg/10 border-b bg-navbar @md:px-6 text-navbar-fg supports-backdrop-filter:backdrop-blur-2xl dark:supports-backdrop-filter:bg-navbar/80",
      inset: [
  ...
  return (
    <Link
      className={composeTailwindRenderProps(
        className,
        "@md:mr-4 flex items-center gap-x-2 @md:px-0 px-2 @md:py-0 py-4 text-fg data-focus-visible:outline-1 data-focus-visible:outline-primary data-focused:outline-hidden",
      )}
      {...props}
    />
  )
...
const Flex = ({ className, ref, ...props }: React.ComponentProps<"div">) => {
  return (
    <div ref={ref} className={twMerge("flex items-center @md:gap-3 gap-2", className)} {...props} />
  )
}
...
const insetStyles = tv({
  ...
  variants: {
    intent: {
      floating: "",
      inset:
        "@md:rounded-lg bg-muted/40 @md:shadow-xs @md:ring-1 @md:ring-fg/15 @md:dark:ring-border",
      navbar: "",
    },
  },
})
...
const Inset = ({ className, ref, ...props }: React.ComponentProps<"div">) => {
  ...
  return (
    <main
      ref={ref}
      data-navbar-intent={intent}
      className={twMerge("flex flex-1 flex-col", intent === "inset" && "@md:px-2 pb-2", className)}
    >
