...
import _memory from "@/resources/data/charts/memory.json"
import { dayFormat } from "@/resources/helpers"
...
export function Memory() {
  ...
  useEffect(() => {
    const isInvalidRange = value.end.compare(value.start) > 30
    if (isInvalidRange) {
    ...
    <Card>
      <Card.Header className="flex-col justify-between gap-2 md:flex-row">
        <div className="max-w-xl flex-1 space-y-1">
          ...
          <Card.Description>
            Shows the amount of RAM being consumed, including allocations by active processes and
            available memory, ensuring efficient application performance and system stability.
          </Card.Description>
        </div>
        <div className="flex items-end gap-2">
          <DateRangePicker
            ...
            maxValue={maxValue}
            validate={(range) =>
              range?.end.compare(range.start) > 30 ? "Maximum stay duration is 1 month." : null
            }
            value={value}
